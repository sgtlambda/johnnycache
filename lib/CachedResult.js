...
class CachedResult {
     ...
     * @Returns {Promise}
     */
    setHashes(cacheableOperation) {
        return Promise.all([
            cacheableOperation.getFileHash(),
            cacheableOperation.getOutputHash()
        ]).then(hashes => {
            this.fileHash   = hashes[0];
            this.outputHash = hashes[1];
            let basename    = slug(sprintf('%s-%s', this.action.substring(0, 16), this.fileHash.substring(0, 16)));
    ...
    static createNew(op) {
        if (op === null)
            ...
            return new CachedResult(null, op.action, null, op.ttl, null, op.compress, null, op.runtime);
    }
