...
class Cache extends EventEmitter {
    ...
    removeResults(cachedResults) {
        var ids = _.map(cachedResults, cachedResult => cachedResult._id);
     ...
     * @return {Promise.<SavedToCache>}
     */
    run(cacheableOperation) {
        let cachedResult, startSave;
        return this.awaitReady()
            ...
            .then(inserted => cachedResult._id = inserted._id)
            .then(() => this.sync())
     ...
     * @returns {Promise.<SavedToCache|RestoredFromCache>}
     */
