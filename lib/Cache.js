...
class Cache extends EventEmitter {
    ...
    getStorageLocation(cachedResult) {
        let ext = cachedResult.compressed ? 'tar.gz' : 'tar';
        return sprintf('%s.%s', path.join(this.workspace, cachedResult.fileName), ext);
    }
    ...
    run(cacheableOperation) {
        ...
        return Promise.all(jobs)
            ...
            .then(() => {
                if (cacheableOperation.onStore)
                    cacheableOperation.onStore(cacheableOperation);
