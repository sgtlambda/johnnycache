...
class Cache extends EventEmitter {
    ...
    storeResult(cacheableOperation) {
        ...
        return this.prepareResult(cacheableOperation)
            .then(result => {
                ...
                return nectar(cacheableOperation.output, writeStream, cacheableOperation.workingDirectory);
            })
            ...
            .then(() => this.sync())
            .then(() => new SavedToCache(cacheableOperation.runtime, Date.now() - startSave, cachedResult));
    }
    ...
    restore(cachedResult) {
        let start = Date.now();
        return extrakt(this.getStorageLocation(cachedResult), this.getWorkingDirectory())
    ...
    getCachedResult(cacheableOperation) {
        ...
        return Promise.all(jobs)
            .then(values => this.index.Result.findOne({where: values[0]}))
