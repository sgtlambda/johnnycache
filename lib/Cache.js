...
class Cache extends EventEmitter {
    ...
    run(cacheableOperation) {
        ...
        return this.awaitReady()
            .then(() => Promise.all([this.prepareResult(cacheableOperation), cacheableOperation.run()]))
            .then(values => {
    ...
    restore(cachedResult) {
        let extract    = tar.extract(this.getExtractPath());
        let readStream = fs.createReadStream(this.getStorageLocation(cachedResult)).pipe(gunzipMaybe());
        readStream.pipe(extract);
        return new Promise(resolve => extract.on('finish', resolve))
            .then(() => cachedResult);
    }
