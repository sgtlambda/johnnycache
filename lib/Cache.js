...
class Cache extends EventEmitter {
    ...
    /**
     * Increment the suffix of the filename on the given cachedResult until no other cachedResult with the same
     * Filename exists
    ...
    async storeResult(cacheableOperation) {
        ...
        const writeStream = this.createWritestream(cacheableOperation, cachedResult);

        await nectar(cacheableOperation.output, writeStream, {cwd: cacheableOperation.workingDirectory});

        await this.assignFileSize(cachedResult);
     ...
     * @returns {Promise}
     */
    async assignFileSize(cachedResult) {

        const stat = await pify(fs.stat)(this.getStorageLocation(cachedResult));
