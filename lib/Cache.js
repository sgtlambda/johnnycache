...
class Cache extends EventEmitter {
    ...
    async writeArchive({cacheableOperation, result}) {
        ...
        const target = splitStream([archive, counter]);
    ...
    async storeResult(cacheableOperation) {
        ...
        return new SavedToCache({
            operationRuntime: cacheableOperation.runtime,
    ...
    async run(cacheableOperation) {
        ...
        const storingResult = new StoringResult({
            cacheableOperation,
            savedToCache: this.storeResult(cacheableOperation),
        });
    ...
    /**
     * Generates the filter callback for retrieving the appropriate cached result document
     * @param {CacheableOperation} op
     * @returns {function}
     */
    static async getQuery(op) {

        const {fileHash, outputHash} = await op.getHashes();

        return new Query({fileHash, outputHash, action: op.action});
    }
    ...
    async getCachedResult(cacheableOperation) {
        ...
        return (doc && await pathExists(this.getAbsolutePath(doc.fileName))) ?
            CachedResult.fromDocument(cacheableOperation, doc) :
            null;
    }
