...
/**
 * @event sync Fired when the index is refreshed
 * @event cleanup Fired when the cache exceeds the maximum allowed size and redundant results are removed from the index
 * @event query Fired when the cache index is queried
 * @event saved Fired when a result was saved to the cache
 */
        ...
        return this.getCachedResult(cacheableOperation)
            ...
            .then(cachedResult => {
                if (cachedResult !== null) {
                    // TODO remove options-level event handlers in favor of eventEmitter-level listeners
